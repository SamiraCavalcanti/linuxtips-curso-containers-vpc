...
resource "aws_route_table" "private_intenet_access_1a" {
  vpc_id = aws_vpc.main.id
  tags = {
    Name = format("%s-private-route-table-1a", var.project_name)
  }
}

resource "aws_route" "private_internet_access_1a" {
  route_table_id         = aws_route_table.private_intenet_access_1a.id
...
resource "aws_route_table" "private_intenet_access_1b" {
  vpc_id = aws_vpc.main.id
  tags = {
    Name = format("%s-private-route-table-1b", var.project_name)
  }
}

resource "aws_route" "private_internet_access_1b" {
  route_table_id         = aws_route_table.private_intenet_access_1b.id
  destination_cidr_block = "0.0.0.0/0"
  gateway_id             = aws_nat_gateway.nat_1b.id
}   

resource "aws_route_table" "private_intenet_access_1c" {
  vpc_id = aws_vpc.main.id
  tags = {
    Name = format("%s-private-route-table-1c", var.project_name)
  }
} 
resource "aws_route" "private_internet_access_1c" {
  route_table_id         = aws_route_table.private_intenet_access_1c.id
  destination_cidr_block = "0.0.0.0/0"
